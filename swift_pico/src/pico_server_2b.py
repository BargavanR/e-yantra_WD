...
# Example adjusted gains
        self.Kp = [22.5, 30.0, 46.8, 12.0]     # Small increase for throttle
        self.Ki = [0.0, 0.0, 0.0, 0.0]        # Slightly reduce Ki for throttle only
        self.Kd = [930.0, 920.0, 1430.0, 80.0]  # Reduce throttle Kd slightly to reduce oscillations
...
'''
    ...
    def pid(self):
        ...
        self.throttle = 1.5*self.Kp[2] * self.error[2] + self.Ki[2] * self.sum_error[2] +1.2*self.Kd[2] * self.diff_error[2]
        ...
        self.cmd.rc_throttle =int(1520 +self.throttle)
    ...
    def execute_callback(self, goal_handle):
        ...
        # Check if hovering is required
        if goal_handle.request.hover:
            self.max_time_inside_sphere = 0
            self.point_in_sphere_start_time = None
            self.time_inside_sphere = 0
            self.duration = self.dtime
            
            # Stabilization loop if hover is required
            while True:
                # Check if drone is within the sphere for stabilization
                drone_is_in_sphere = self.is_drone_in_sphere(self.drone_position, goal_handle, 0.6)
                
                if not drone_is_in_sphere:
                    self.point_in_sphere_start_time = None  # reset if not in the sphere
                else:
                    # Calculate time inside sphere if within range
            ...
            # Move to the next waypoint immediately without hovering
            self.get_logger().info('Continuing to next waypoint without hovering...')
        
        # Finish and return result
        goal_handle.succeed()
        result = NavToWaypoint.Result()
        result.hov_time = self.dtime - self.duration if goal_handle.request.hover else 0
        return result
